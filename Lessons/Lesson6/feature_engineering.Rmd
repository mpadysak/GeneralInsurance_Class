...
dt_pol_w_claims <- readRDS("C:\\Users\\mpady\\OneDrive\\Dokumenty\\GeneralInsurance_Class\\Data\\lesson6_dt_pol_w_claims.rds")
...
```{r}
source("C:\\Users\\mpady\\OneDrive\\Dokumenty\\GeneralInsurance_Class\\Lessons\\Lesson6\\Support\\emb_chart.R")
emblem_graph(
...
#Moj model, treningova vzorka
model4 <- glm(data = train,
              formula = Burning_Cost ~ Veh_type1 + Construct_year,
              family = Gamma())
prediction <- predict(model4, val, type = "response")
summary(model4)
...
model5 <- glm(data = train,
              ...
              family = Gamma())
prediction <- predict(model5, val, type = "response")
summary(model5)

mse(predict(model5, train, type = "response"), train$Burning_Cost)
mse(predict(model5, val, type = "response"), val$Burning_Cost) 
#RMSE kleslo, ale nie o moc... aj to sa pocita .-) Model by mal byt teda robustnejsi aj co sa tyka trenovanej vzorky aj co sa tyka predikcii "out of sample"
...
#zgrupoval by som vodicov mladsich ako 30, vratane lebo je tam malo pozorovani a vyskakuje to v grafe
#tiez by som spravil skupinu od 67 tiez kvoli poctu pozorovani a je to take logicke zaclenenie do dochodkoveho veku
train <- train %>% 
  mutate(D_age = ifelse(D_age <= 30, 30, D_age))
train <- train %>% 
  mutate(D_age = ifelse(D_age >= 67, 70, D_age)) 
...
#Uz to vyzera lepsie 
#Prva technika bola pridanie dalsej premennej ale tak aby to nebolo overfitted ale naopak aby kleslo RMSE, dalo by sa este poskusat ktora kombinacia premennych je najlepsia out-of-sample ale myslim ze ako myslienka to staci
#Potom som skusil zgrupovat nejake skupiny vekove aby neboli v grafe "spikes" a vznikli chlieviky s viacej skupinami a aby som dostal lepsie actual mean voci fitted, aj to celkom zafungovalo este by to mozno chcelo nejake vekove kategorie zlucit.. 
